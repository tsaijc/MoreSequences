...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(100.0, 40.0)]  # All numbers are approximate.
    circle = rg.Circle(rg.Point(80, 40), 20)
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  14 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(140, 140), 100)
    draw_points_on_circle(window, circle, 14, 'blue')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  14 slices, thin (thickness=2) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 140)
    circle.outline_thickness = 2
    pizza(window, circle, 14, 'red', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  14 segments with thin green lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(140, 140), 200)
    circle.outline_thickness = 3
    polygon(window, circle, 14, 'green', 2)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  14 yellow lines on pink circle, hops = 10.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'pink'
    fancy_polygon(window, circle, 14, 10, 'yellow', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    index = 0
    upperindex = index + hops_to_next_point
    circle.attach_to(window)
    while(index < number_of_lines):
        line = rg.Line(points[index], points[upperindex])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
